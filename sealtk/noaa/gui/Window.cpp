...
struct WindowData
{
  ...
  sealtk::gui::CreateTrackPlayerTool* createTrackTool = nullptr;
    ...
    d_ptr{new WindowPrivate{this}}
{
  ...
  connect(d->ui.actionCreateDetection, &QAction::triggered,
          this, [d]{
            d->eoWindow.player->setActiveTool(d->eoWindow.createTrackTool);
            d->irWindow.player->setActiveTool(d->irWindow.createTrackTool);
          });
                                 ...
                                 sealtk::noaa::gui::Player::Role role)
{
  ...
  data->createTrackTool = new sg::CreateTrackPlayerTool{data->player};
  ...
  QObject::connect(
    data->createTrackTool, &sg::CreateTrackPlayerTool::detectionCreated,
...
void WindowPrivate::createDetection(WindowData* data, QRectF const& detection)
{
  ...
  auto* const tracks = qobject_cast<sc::KwiverTrackModel*>(
    data->trackModel.get());
  ...
  if (tracks)
  {
    ...
    {
      auto const index = tracks->index(row, 0);
      maxId = std::max(
        maxId, tracks->data(index, sc::LogicalIdentityRole).toLongLong());
    }

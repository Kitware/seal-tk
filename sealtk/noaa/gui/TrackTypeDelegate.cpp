...
void TrackTypeDelegate::setEditorData(
  QWidget* editor, QModelIndex const& pi) const
{
  if (auto* const pm = qobject_cast<QAbstractProxyModel const*>(pi.model()))
  {
    if (auto* const sm = pm->sourceModel())
    {
      auto const& si = pm->mapToSource(pi);
      auto const& type = pm->data(pi, core::ClassificationTypeRole).toString();
...
void TrackTypeDelegate::setModelData(
  QWidget* editor, QAbstractItemModel* model, QModelIndex const& pi) const
{
  if (auto* const pm = qobject_cast<QAbstractProxyModel*>(model))
  {
    if (auto* const sm = pm->sourceModel())
    {
      auto const& si = pm->mapToSource(pi);
      ...
      sm->setData(si, newClassification, core::ClassificationRole);
    }
  }
}
